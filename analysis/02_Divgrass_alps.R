...
####Function Insu----
  ...
  cbind.data.frame(D_insu=D_insu,D_thr=D_thr,insured=insured,insurer=insurer,Div_insured=length(spe_insured),
                   ...
                   spnotinsu=spnotinsu,spinsu=spinsu)
...
####LOAD POINTS DATA & SHAPE_FILE & SUBSET FRENCH ALPS----
  ...
  # Load shapefile and convert to Lambert-93 immediately
    shp_file <- sf::st_read(here::here("data","eea_v_3035_1_mio_biogeo-regions_p_2016_v01_r00","BiogeoRegions2016.shp"))
  ...
  # Remove NAs in coordinates and convert to sf object (already in Lambert-93)
    points_sf <- divgrass_com[!is.na(divgrass_com$X_L93) & !is.na(divgrass_com$Y_L93), ]
    points_sf <- sf::st_as_sf(points_sf,coords = c("X_L93", "Y_L93"), crs = crs_lambert93)
  ...
  # Define bounding box for the French Alps (approximate) and transform to Lambert-93
    bbox_french_alps <- sf::st_bbox(c(xmin = 4.95, xmax = 7.85, ymin = 43.85, ymax = 46.5), crs = 4326) %>%
      sf::st_as_sfc() %>%
  ...
  # Filter points inside the French Alps Box 
    Div_grass_french_alps <- points_sf_unique[sf::st_within(points_sf_unique, french_alps, sparse = FALSE), ]    
  
  # Remove points that are in Italy
    sites_italy <- c('SOP_T_1937.3.1','SOP_T_1937.4.1','SOP_T_1937.5.1','SOP_T_2303.5.1','SOP_T_862.2.7',
                     'SOP_T_491.2.6','SOP_T_2303.4.1','SOP_T_491.8.5','SOP_T_1891.24.4','SOP_T_1194.3.20',
                     'SOP_T_194.7.19','SOP_T_194.7.20','SOP_T_1194.4.9','CME1_C16041898','CME1_C16046914')
    
    Div_grass_french_alps <- Div_grass_french_alps[!Div_grass_french_alps$com %in% sites_italy, ]

  # Keep only points with gps precision >1137 meters 
    Div_grass_french_alps <- Div_grass_french_alps[Div_grass_french_alps$PREC_LOC%in%c('704','1137'),]
  ...
  #Map 
    ...
    # 
  #Save & Load 
    save(Div_grass_french_alps,file=here::here("results","Div_grass_french_alps.Rdata"))
...
#Long, run once (and load)
  ...
  # Find neighbors within radius for each point in Div_grass_french_alps (this is long)
    alpine_within_radius <- sf::st_is_within_distance(Div_grass_french_alps, Div_grass_french_alps, dist = radius)
    save(alpine_within_radius,file=here::here("results","alpine_within_radius.Rdata"))
    
  # Create a data frame showing which points are neighbors
    alpine_buffer <- data.frame(
      focal_cell = rep(Div_grass_french_alps$com, sapply(alpine_within_radius, length)),  # Use 'com' as ID
...
#Select the traits 
    ...
    filled_species_traits_wide <- filled_species_traits_wide[,c("TAXREF_SHORT","SLA", "PH", "SM", "LA", "LDMC")]
    
    transformed_species_traits <- na.omit(data.frame(filled_species_traits_wide$TAXREF_SHORT, 
    ...
    transformed_species_traits <- transformed_species_traits[!transformed_species_traits$taxa.name%in%"POTAMOGETON PECTINATUS",]
...
#OCCURENCES----
    ...
    #remove species without traits 
      divgrass_alps <- divgrass_alps[divgrass_alps$TAXREF_SHORT%in%transformed_species_traits$taxa.name,]
    
    #compute a list of sites with species composition 
    
    all_com_divgrass <- pbmcapply::pbmclapply(coms,function(id_site){
...
#Compute the trait distance matrix and distinctiveness on the alp plants
  ...
  #Keep only species within alps 
    alps_transformed_species_traits <- transformed_species_traits[transformed_species_traits$taxa.name%in%unique(divgrass_alps$TAXREF_SHORT),]
    
  #Distance matrix and distinctiveness
    disttrait_divgrass = funrar::compute_dist_matrix(alps_transformed_species_traits[,-1],metric = "euclidean")
    save(disttrait_divgrass,file=here::here("results","disttrait_divgrass.Rdata"))
  ...
  ggsave(file=here::here("outputs","S2_Fig_1.tiff"), S2_Fig_1,width = 20, height = 10, dpi = 300, units = "cm", device='tiff') 
...
####LOOP OVER MANY CELLS WITH BUFFERS, Fig_4b,d----
  ...
  #Compute insurance 
  #long to run 
    divgrass_insurance <- do.call(rbind,pbmcapply::pbmclapply(sp_cell, function(focal_cell){
      
      #focal_cell=sp_cell[600]
      
      buffer_cells <- cbind.data.frame(reference=focal_cell,
                                       neighbor=alpine_buffer$neighbor_cell[alpine_buffer$focal_cell%in%focal_cell])
      ...
      buffer_cell_used <- buffer_cells[unlist(lapply(as.character(buffer_cells$neighbor),function(id){length(unique(all_com_divgrass[[id]]))>=5})),]
      
      #Compute insurance when there is more than 5 species in the focal cell
      if ((length(sp_focal)>=5) & (nrow(buffer_cell_used)>2)){
        
        I_buffer <- do.call(rbind,(pbmcapply::pbmclapply(2:dim(buffer_cell_used)[1],function(j){
          
          #j=2
          
          a <- Insu(insured=focal_cell,insurer=buffer_cell_used$neighbor[j],occ_list=all_com_divgrass,dist_mat=dist_mat,D_insu,D_thr)
          b <- Insu(insured=buffer_cell_used$neighbor[j],insurer=focal_cell,occ_list=all_com_divgrass,dist_mat=dist_mat,D_insu,D_thr)
          a$Outward <- b$Inward
          a
        ...
        #allspecies 
        all_species <- unique(unlist(lapply(buffer_cell_used$neighbor, function(id){occ_list[[id]]})))
        
        #who is never insured
        sp_insu=table(unlist(strsplit(I_buffer$spinsu[!I_buffer$spinsu==""],",")))
        sp_insu <- names(sp_insu[sp_insu>0])
        sp_notinsu=table(unlist(strsplit(I_buffer$spnotinsu[!I_buffer$spnotinsu==""],",")))
        sp_notinsu <- names(sp_notinsu[sp_notinsu>0])
        sp_notinsu <- paste0(sp_notinsu[!sp_notinsu%in%sp_insu],collapse = ",")
  ...
  #Add env values (not be used)
    ...
    divgrass_insurance <- merge(divgrass_insurance,divgrass_com,by = 'row.names', all = FALSE)
    divgrass_insurance <- sf::st_as_sf(divgrass_insurance, coords = c("X_L93", "Y_L93"), crs = 2154)
  
  #Compute quantiles for classification in Source, Sink, and Intermediate
    low_threshold <- quantile(divgrass_insurance$Inward - divgrass_insurance$Outward, 0.1, na.rm = TRUE)
    high_threshold <- quantile(divgrass_insurance$Inward - divgrass_insurance$Outward, 0.9, na.rm = TRUE)
    
    divgrass_insurance$Category <- ifelse(divgrass_insurance$Inward - divgrass_insurance$Outward >= high_threshold, "Sink",
                                          ifelse(divgrass_insurance$Inward - divgrass_insurance$Outward <= low_threshold, "Source", "Intermediate"))
  ...
  #Save
    save(divgrass_insurance,file=here::here("results","divgrass_insurance.RData"))
  
  #Load
    divgrass_insurance <- get(load(here::here('results','divgrass_insurance.RData')))
  ...
  #plot inward vs outward Fig_4b_right
    ...
    all_cells <- divgrass_insurance[order(divgrass_insurance$Nsp_loc,decreasing = F),]
    all_cells <- divgrass_insurance[sample(1:nrow(divgrass_insurance),nrow(divgrass_insurance)),]
    all_cells$log10_Nsp_loc=log10(all_cells$Nsp_loc)
    
    Fig_4b_right <- ggplot(all_cells, aes(x=Inward, y=Outward, color=log10_Nsp_loc)) +
      ...
      geom_abline (slope=1, intercept=as.numeric(quantile(all_cells$Outward-all_cells$Inward,prob=0.8,na.rm = T)),linetype = "dashed", size=0.5)+
      geom_abline (slope=1, intercept=as.numeric(quantile(all_cells$Outward-all_cells$Inward,prob=0.2,na.rm = T)),linetype = "dashed", size=0.5)+
  ...
  #map source and sinks
    ...
    # Convert to factor for proper mapping
      divgrass_insurance$Category <- factor(divgrass_insurance$Category, levels = c("Source", "Sink", "Intermediate"))
    
    # Define custom colors for the 3 categories
      category_palette <- c("Sink" = "#FA6C67", "Intermediate" = "#FFF7BA", "Source" = "#52A153")
    
    # look at the categories to check Fig_S2_Fig_2
      S2_Fig_2 <- ggplot(divgrass_insurance, aes(x = Inward, y = Outward, color = Category)) +
      ...
      ggsave(file=here::here("outputs","S2_Fig_2.tiff"), S2_Fig_2,width = 13, height = 9, dpi = 300, units = "cm", device='tiff') 
    ...
    # Create the map for all categories with an example cell
      subper=1
      sub_divgrass_insurance <- divgrass_insurance[sample(1:nrow(divgrass_insurance),round(nrow(divgrass_insurance)*subper)),]
  ...
  #Create the map for only sources and sinks only Fig_3a
    ...
    filtered_divgrass_insurance <- sub_divgrass_insurance[sub_divgrass_insurance$Category %in% c("Source", "Sink"), ]
    filtered_divgrass_insurance$Category <- factor(filtered_divgrass_insurance$Category, levels = c("Source", "Sink"))
    ...
    #Box plot categories Fig4bleft
      ...
      divgrass_insurance$Category <- factor(divgrass_insurance$Category, levels = c("Source", "Intermediate", "Sink"))
      
      Fig_4b_left <- ggplot(divgrass_insurance, aes(x = Category, y = Nsp_loc,fill = Category))+
      ...
      ggsave(file=here::here("outputs","Fig_4b.tiff"), Fig_4b,width = 15, height = 7, dpi = 300, units = "cm", device='tiff') 
    
  #What are the species not insured and in how much plots ? 
      
    sp_notinsu <- unique(unlist(strsplit(divgrass_insurance$sp_notinsu[!divgrass_insurance$sp_notinsu==""],",")))
    
    sp_notinsu_tb <- table(unlist(strsplit(divgrass_insurance$sp_notinsu[!divgrass_insurance$sp_notinsu==""],",")))
  ...
  #plot the pcoa Fig_4d
        ...
        #pc_axes_sp_notinsu
          ...
          pc_axes_sp_notinsu$Distance <- sqrt(pc_axes_sp_notinsu$Axis1^2 + pc_axes_sp_notinsu$Axis2^2)
          pc_axes_sp_notinsu_radius <- min(pc_axes_sp_notinsu$Distance)
          pc_axes_sp_notinsu <- pc_axes_sp_notinsu[order(pc_axes_sp_notinsu$pernotinsu,decreasing = F),]
        
        #pc_axes_distinct
          pc_axes_distinct <- pc_axes[rownames(pc_axes)%in%insured_D,]
          pc_axes_distinct$Distance <- sqrt(pc_axes_distinct$Axis1^2 + pc_axes_distinct$Axis2^2)
          pc_axes_distinct_radius <- min(pc_axes_distinct$Distance)
        
        #target some species for illustration 
          focal_species <- c("BUPLEURUM BALDENSE", "LATHYRUS LATIFOLIUS", 
                             "PETASITES HYBRIDUS", "DIPHASIASTRUM ALPINUM")
        ...
        #plot the pcoa
          ...
          Fig_4d <- ggplot(pc_axes, aes(x = Axis1, y = Axis2))+
            ...
            xlim(min(pcoas_divgrass$vectors[,1])-1.2,max(pcoas_divgrass$vectors[,1])+1.7)+
            theme_bw()+
            geom_point(data = pc_axes_sp_notinsu, 
                       aes(color = pernotinsu), size = ptsize-0.2) +
            scale_color_gradient(low = "lightblue", high = "darkblue")+
            geom_point(data = pc_axes_distinct,shape = 1, 
                       col = "#FA6C67",size=ptsize)+
            geom_point(data = focal_coords, 
                      shape = 1, color = "#FA6C67", size = ptsize + 2, stroke = 1.15)+
            theme(legend.position = "none",
                  axis.title.x=element_blank(),
        ...
        ggsave(file=here::here("outputs","Fig_4d.tiff"), Fig_4d,width = 14, height = 12, dpi = 300, units = "cm", device='tiff') 
      
      #find examples 
        rownames(pc_axes_sp_notinsu) <- pc_axes_sp_notinsu$Species
        plot(pc_axes_sp_notinsu$Axis1, pc_axes_sp_notinsu$Axis2, pch = 19, col = "blue", main = "Click on a point to get its ID")
        clicked_points <- identify(pc_axes_sp_notinsu$Axis1, pc_axes_sp_notinsu$Axis2, labels = rownames(pc_axes_sp_notinsu), plot = TRUE)
        print(paste("You clicked on ID(s):", rownames(pc_axes_sp_notinsu)[clicked_points]))
        pc_axes_sp_notinsu[pc_axes_sp_notinsu$Species%in%rownames(pc_axes_sp_notinsu)[clicked_points],]
#---- 
...
####ONE CELL WITHIN A BUFFER----
      ...
      I_buffer <- do.call(rbind,(pbmcapply::pbmclapply(1:dim(buffer_cell_used)[1],function(j){
      ...
      insuwho <- pbmcapply::pbmclapply(2:dim(buffer_cell_used)[1],function(j){
        #j=2
        
        Insu_who(insured=focal_cell,insurer=buffer_cell_used$neighbor[j],occ_list=all_com_divgrass,dist_mat=dist_mat,D_insu,D_thr)
      },mc.cores = parallel::detectCores())
      ...
      for(i in 1:nrow(neighbors)){
        neighbors$outward[i]=I_buffer$Outward[I_buffer$insurer%in%neighbors$com[i]]
      }
      ...
      tmap::tm_shape(Div_grass_french_alps) +
        ...
        tmap::tm_dots(fill = "gray", size = 0.5)+
        tmap::tm_shape(neighbors) + 
        tmap::tm_symbols(col = "inward", palette = palette, size = 0.4, fill.scale = "cont") +
        tmap::tm_shape(reference_point) + 
        tmap::tm_dots(fill = "red", size = 0.5)  # Plot the selected reference point
...
####TWO CELLS Fig 4c----
  ...
  library(ggforce)
    ...
    pc_insured <- data.frame(pcoas_divgrass$vectors[rownames(pcoas_divgrass$vectors)%in%spe_insured,])
    pc_insurer <- data.frame(pcoas_divgrass$vectors[rownames(pcoas_divgrass$vectors)%in%spe_insurer,])
  ...
  #Project the species on the global PCA 
    ...
    fig_sink <- ggplot(pc_axes, aes(x = Axis1, y = Axis2))+
        ...
        geom_vline(xintercept = 0,linetype="dashed",col="#9E9E9E")+
        geom_hline(yintercept = 0,linetype="dashed",col="#9E9E9E")+
        ylim(min(pcoas_divgrass$vectors[,2])+addymin,max(pcoas_divgrass$vectors[,2])-remymax)+
        xlim(min(pcoas_divgrass$vectors[,1])+addxmin,max(pcoas_divgrass$vectors[,1])-remxmax)+
        theme_bw()+
        geom_point(data = pc_insured, 
                   col = "black",size=ptsize)+
        geom_point(data = pc_insured_Di_Q, 
                   col = "#FA6C67",size=ptsize)+
        geom_circle(aes(x0 = Axis1, y0 = Axis2, r = D_insu), data = pc_insurer_Di_Q,
                    col = "#52A153",linetype=2)+
    ...
    ggsave(file=here::here("outputs","Fig_4c.tiff"), Insured_Insurer,width = 7, height = 14, dpi = 300, units = "cm", device='tiff')
