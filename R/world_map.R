...
world_map <- function(id=NA,color_s=RColorBrewer::brewer.pal(name = "YlOrRd", 9)) {
  #id=NA
  ...
  raster::plot(richness_robinson, asp = 1, axes = FALSE, legend = FALSE,
               legend.mar = 0, legend.width = 0, col = color_s)
  ...
  if (!is.na(id)){
    for (i in id){
      ...
      if (!is.na(xy[1, "x"])) {
        ...
        plot(sf::st_geometry(xy_buf), asp = 1, border = "black", 
             col = "#00000044", add = TRUE) 
      }
    }
  }
...
world_map_robin <- function(id=NA,color_s=RColorBrewer::brewer.pal(name = "YlOrRd", 9)) {
  ## Layer to map ----
  
  richness <- suppressWarnings(
    terra::rast(here::here("data","Loiseau2021",
                              paste0(options()$"taxa", "_richness.tif"))))
  ...
  terra::plot(richness_robinson, asp = 1, axes = FALSE, legend = FALSE,
               legend.mar = 0, legend.width = 0, col = color_s)
  if (!is.na(id)){
  for (i in id){
...
world_map2 <- function(data,color_s) {
  ...
  if (ncol(data) != 2) {
    stop("Argument 'data' must have exactly 2 columns (cell id and variable ", 
         "to map)")
  }
  ...
  values <- data.frame("real_cell_id" = 1:raster::ncell(grd), 
                       "old_cell_id"  = grd[])
  values <- na.omit(values)
  
  data <- merge(values, data, by.x = "old_cell_id", by.y = "old_cell_id", 
                all = FALSE)
  ...
  raster::plot(ras, asp = 1, axes = FALSE, legend = FALSE,
               legend.mar = 0, legend.width = 0, col = color_s)
