...
beak_culmen_split <- merge(
  beak_culmen_M1,
  beak_culmen_M2,
  by = "Unique.Specimen.Identifier"
)
beak_culmen_split <- beak_culmen_split[complete.cases(beak_culmen_split), ]

#Calculate, mean, sd and cv.
beak_culmen_split$mean <- apply(
  beak_culmen_split[, c("Beak.culmen.x", "Beak.culmen.y")],
  1,
  mean
)
beak_culmen_split$sd <- apply(
  beak_culmen_split[, c("Beak.culmen.x", "Beak.culmen.y")],
  1,
  sd
)
...
beak_culmen_cor <- round(
  cor(beak_culmen_split$Beak.culmen.x, beak_culmen_split$Beak.culmen.y),
  3
)
...
beak_culmen_diffp <- ((beak_culmen_post - beak_culmen_baseline) /
  beak_culmen_baseline) +
  1
beak_culmen_sd.diff <- sd(beak_culmen_diff)
beak_culmen_sd.diffp <- sd(beak_culmen_diffp)
beak_culmen_BA <- data.frame(
  beak_culmen_baseline,
  beak_culmen_post,
  beak_culmen_diff,
  beak_culmen_diffp
...
label_beak_culmen <- substitute(
  paste(italic("R"["c"]), " = ", cv, ", ", italic("n"), " = ", n),
...
scatter_beak_culmen <-
  ...
  geom_abline(
    intercept = mean(beak_culmen_BA$beak_culmen_diffp),
    colour = "blue",
    linetype = 1,
    size = 1.2
  ) +
...
diffplot_beak_culmen <- ggplot(
  beak_culmen_BA,
  aes(beak_culmen_baseline, beak_culmen_diffp)
) +
  geom_point(size = 2, colour = rgb(0, 0, 0, alpha = 0.5)) +
  geom_hline(
    yintercept = mean(beak_culmen_BA$beak_culmen_diffp),
    colour = "blue",
    linetype = 1,
    size = 1
  ) +
...
#Calculate, mean, sd and cv.
beak_nares_split$mean <- apply(
  beak_nares_split[, c("Beak.nares.x", "Beak.nares.y")],
  1,
  mean
)
beak_nares_split$sd <- apply(
  beak_nares_split[, c("Beak.nares.x", "Beak.nares.y")],
  1,
  sd
)
...
beak_nares_cor <- round(
  cor(beak_nares_split$Beak.nares.x, beak_nares_split$Beak.nares.y),
  3
)
...
beak_nares_diffp <- ((beak_nares_post - beak_nares_baseline) /
  beak_nares_baseline) +
  1
beak_nares_sd.diff <- sd(beak_nares_diff)
beak_nares_sd.diffp <- sd(beak_nares_diffp)
beak_nares_BA <- data.frame(
  beak_nares_baseline,
  beak_nares_post,
  beak_nares_diff,
  beak_nares_diffp
...
scatter_beak_nares <-
  ...
  labs(x = "Measurer 2", y = "Measurer 1") +
  geom_abline(
    intercept = mean(beak_nares_BA$beak_nares_diffp),
    colour = "blue",
    linetype = 1,
    size = 1.2
  ) +
...
#Create Bland-Altman plot to illustrate proportional differencers between specimen Measurerments
diffplot_beak_nares <- ggplot(
  beak_nares_BA,
  aes(beak_nares_baseline, beak_nares_diffp)
) +
  geom_point(size = 2, colour = rgb(0, 0, 0, alpha = 0.5)) +
  geom_hline(
    yintercept = mean(beak_nares_BA$beak_nares_diffp),
    colour = "blue",
    linetype = 1,
    size = 1
  ) +
...
#Calculate, mean, sd and cv.
beak_width_split$mean <- apply(
  beak_width_split[, c("Beak.width.x", "Beak.width.y")],
  1,
  mean
)
beak_width_split$sd <- apply(
  beak_width_split[, c("Beak.width.x", "Beak.width.y")],
  1,
  sd
)
...
beak_width_cor <- round(
  cor(beak_width_split$Beak.width.x, beak_width_split$Beak.width.y),
  3
)
...
beak_width_diffp <- ((beak_width_post - beak_width_baseline) /
  beak_width_baseline) +
  1
beak_width_sd.diff <- sd(beak_width_diff)
beak_width_sd.diffp <- sd(beak_width_diffp)
beak_width_BA <- data.frame(
  beak_width_baseline,
  beak_width_post,
  beak_width_diff,
  beak_width_diffp
...
scatter_beak_width <-
  ...
  labs(x = "Measurer 2", y = "Measurer 1") +
  geom_abline(
    intercept = mean(beak_width_BA$beak_width_diffp),
    colour = "blue",
    linetype = 1,
    size = 1.2
  ) +
...
#Create Bland-Altman plot to illustrate proportional differencers between specimen Measurerments
diffplot_beak_width <- ggplot(
  beak_width_BA,
  aes(beak_width_baseline, beak_width_diffp)
) +
  geom_point(size = 2, colour = rgb(0, 0, 0, alpha = 0.5)) +
  geom_hline(
    yintercept = mean(beak_nares_BA$beak_nares_diffp),
  ...
  geom_hline(yintercept = 1, colour = "red", size = 1, linetype = "dashed") +
  geom_hline(
    yintercept = mean(beak_width_BA$beak_width_diffp) +
      1.96 * beak_width_sd.diffp,
    linetype = 2
  ) +
  geom_hline(
    yintercept = mean(beak_width_BA$beak_width_diffp) -
...
#Calculate, mean, sd and cv.
beak_depth_split$mean <- apply(
  beak_depth_split[, c("Beak.depth.x", "Beak.depth.y")],
  1,
  mean
)
beak_depth_split$sd <- apply(
  beak_depth_split[, c("Beak.depth.x", "Beak.depth.y")],
  1,
  sd
)
...
beak_depth_cor <- round(
  cor(beak_depth_split$Beak.depth.x, beak_depth_split$Beak.depth.y),
  3
)
...
beak_depth_diffp <- ((beak_depth_post - beak_depth_baseline) /
  beak_depth_baseline) +
  1
beak_depth_sd.diff <- sd(beak_depth_diff)
beak_depth_sd.diffp <- sd(beak_depth_diffp)
beak_depth_BA <- data.frame(
  beak_depth_baseline,
  beak_depth_post,
  beak_depth_diff,
  beak_depth_diffp
...
scatter_beak_depth <-
  ...
  labs(x = "Measurer 2", y = "Measurer 1") +
  geom_abline(
    intercept = mean(beak_depth_BA$beak_depth_diffp),
    colour = "blue",
    linetype = 1,
    size = 1.2
  ) +
...
#Create Bland-Altman plot to illustrate proportional differencers between specimen Measurerments
diffplot_beak_depth <- ggplot(
  beak_depth_BA,
  aes(beak_depth_baseline, beak_depth_diffp)
) +
  geom_point(size = 2, colour = rgb(0, 0, 0, alpha = 0.5)) +
  geom_hline(
    yintercept = mean(beak_nares_BA$beak_nares_diffp),
  ...
  geom_hline(yintercept = 1, colour = "red", size = 1, linetype = "dashed") +
  geom_hline(
    yintercept = mean(beak_depth_BA$beak_depth_diffp) +
      1.96 * beak_depth_sd.diffp,
    linetype = 2
  ) +
  geom_hline(
    yintercept = mean(beak_depth_BA$beak_depth_diffp) -
...
ggsave(
  filename = paste0(figFolder, 'Fig S1 - Beak traits.pdf'),
  width = 14,
  height = 14,
  dpi = 600,
  units = "in",
...
ggsave(
  filename = paste0(figFolder, 'Fig S1 - Beak traits.jpg'),
  width = 14,
  height = 14,
  dpi = 600,
  units = "in",
...
tarsus_length_split <- merge(
  tarsus_length_M1,
  tarsus_length_M2,
  by = "Unique.Specimen.Identifier"
)
tarsus_length_split <- tarsus_length_split[
  complete.cases(tarsus_length_split),
]

#Calculate, mean, sd and cv.
tarsus_length_split$mean <- apply(
  tarsus_length_split[, c("tarsus.length.x", "tarsus.length.y")],
  1,
  mean
)
tarsus_length_split$sd <- apply(
  tarsus_length_split[, c("tarsus.length.x", "tarsus.length.y")],
  1,
  sd
)
tarsus_length_split$cv <- (tarsus_length_split$sd / tarsus_length_split$mean) *
  100
...
tarsus_length_cor <- round(
  cor(tarsus_length_split$tarsus.length.x, tarsus_length_split$tarsus.length.y),
  3
)
...
tarsus_length_diffp <- ((tarsus_length_post - tarsus_length_baseline) /
  tarsus_length_baseline) +
  1
tarsus_length_sd.diff <- sd(tarsus_length_diff)
tarsus_length_sd.diffp <- sd(tarsus_length_diffp)
tarsus_length_BA <- data.frame(
  tarsus_length_baseline,
  tarsus_length_post,
  tarsus_length_diff,
  tarsus_length_diffp
...
#Create specific text functions
label_tarsus_length <- substitute(
  paste(italic("R"["c"]), " = ", cv, ", ", italic("n"), " = ", n),
...
scatter_tarsus_length <-
  ...
  labs(x = "Measurer 2", y = "Measurer 1") +
  geom_abline(
    intercept = mean(tarsus_length_BA$tarsus_length_diffp),
    colour = "blue",
    linetype = 1,
    size = 1.2
  ) +
...
#Create Bland-Altman plot to illustrate proportional differencers between specimen Measurerments
diffplot_tarsus_length <- ggplot(
  tarsus_length_BA,
  aes(tarsus_length_baseline, tarsus_length_diffp)
) +
  geom_point(size = 2, colour = rgb(0, 0, 0, alpha = 0.5)) +
  geom_hline(
    yintercept = mean(tarsus_length_BA$tarsus_length_diffp),
    colour = "blue",
    linetype = 1,
    size = 1
  ) +
...
#Calculate, mean, sd and cv.
wing_length_split$mean <- apply(
  wing_length_split[, c("wing.length.x", "wing.length.y")],
  1,
  mean
)
wing_length_split$sd <- apply(
  wing_length_split[, c("wing.length.x", "wing.length.y")],
  1,
  sd
)
...
wing_length_cor <- round(
  cor(wing_length_split$wing.length.x, wing_length_split$wing.length.y),
  3
)
...
wing_length_diffp <- ((wing_length_post - wing_length_baseline) /
  wing_length_baseline) +
  1
wing_length_sd.diff <- sd(wing_length_diff)
wing_length_sd.diffp <- sd(wing_length_diffp)
wing_length_BA <- data.frame(
  wing_length_baseline,
  wing_length_post,
  wing_length_diff,
  wing_length_diffp
...
scatter_wing_length <-
  ...
  labs(x = "Measurer 2", y = "Measurer 1") +
  geom_abline(
    intercept = mean(wing_length_BA$wing_length_diffp),
    colour = "blue",
    linetype = 1,
    size = 1.2
  ) +
...
#Create Bland-Altman plot to illustrate proportional differencers between specimen Measurerments
diffplot_wing_length <- ggplot(
  wing_length_BA,
  aes(wing_length_baseline, wing_length_diffp)
) +
  geom_point(size = 2, colour = rgb(0, 0, 0, alpha = 0.5)) +
  geom_hline(
    yintercept = mean(wing_length_BA$wing_length_diffp),
    colour = "blue",
    linetype = 1,
    size = 1
  ) +
...
scatter_kipps <-
  ...
  labs(x = "Measurer 2", y = "Measurer 1") +
  geom_abline(
    intercept = mean(kipps_BA$kipps_diffp),
    colour = "blue",
    linetype = 1,
    size = 1.2
  ) +
...
diffplot_kipps <- ggplot(kipps_BA, aes(kipps_baseline, kipps_diffp)) +
  geom_point(size = 2, colour = rgb(0, 0, 0, alpha = 0.5)) +
  geom_hline(
    yintercept = mean(kipps_BA$kipps_diffp),
    colour = "blue",
    linetype = 1,
    size = 1
  ) +
  geom_hline(yintercept = 1, colour = "red", size = 1, linetype = "dashed") +
  geom_hline(
    yintercept = mean(kipps_BA$kipps_diffp) + 1.96 * kipps_sd.diffp,
    linetype = 2
  ) +
  geom_hline(
    yintercept = mean(kipps_BA$kipps_diffp) - 1.96 * kipps_sd.diffp,
...
#Calculate, mean, sd and cv.
tail_length_split$mean <- apply(
  tail_length_split[, c("tail.length.x", "tail.length.y")],
  1,
  mean
)
tail_length_split$sd <- apply(
  tail_length_split[, c("tail.length.x", "tail.length.y")],
  1,
  sd
)
...
tail_length_cor <- round(
  cor(tail_length_split$tail.length.x, tail_length_split$tail.length.y),
  3
)
...
tail_length_diffp <- ((tail_length_post - tail_length_baseline) /
  tail_length_baseline) +
  1
tail_length_sd.diff <- sd(tail_length_diff)
tail_length_sd.diffp <- sd(tail_length_diffp)
tail_length_BA <- data.frame(
  tail_length_baseline,
  tail_length_post,
  tail_length_diff,
  tail_length_diffp
...
scatter_tail_length <-
  ...
  labs(x = "Measurer 2", y = "Measurer 1") +
  geom_abline(
    intercept = mean(tail_length_BA$tail_length_diffp),
    colour = "blue",
    linetype = 1,
    size = 1.2
  ) +
...
#Create Bland-Altman plot to illustrate proportional differencers between specimen Measurerments
diffplot_tail_length <- ggplot(
  tail_length_BA,
  aes(tail_length_baseline, tail_length_diffp)
) +
  geom_point(size = 2, colour = rgb(0, 0, 0, alpha = 0.5)) +
  geom_hline(
    yintercept = mean(tail_length_BA$tail_length_diffp),
    colour = "blue",
    linetype = 1,
    size = 1
  ) +
...
######################################################################
plot_grid(
  combined_tarsus_length,
  combined_tail_length,
  combined_wing_length,
  combined_kipps,
  ncol = 2,
  nrow = 2
)
...
ggsave(
  filename = paste0(figFolder, 'Fig S2 - Body traits.pdf'),
  width = 14,
  height = 14,
  dpi = 400,
  units = "in",
...
ggsave(
  filename = paste0(figFolder, 'Fig S2 - Body traits.jpg'),
  width = 14,
  height = 14,
  dpi = 600,
  units = "in",
