...
extant_species <- read.csv(
  paste0(TraitDataFolder, 'AVONET_Extant_Species_List.csv'),
  h = T
)
...
mean_museum <- museum %>%
  ...
  summarise(across(
    c("Beak.Length_Culmen", "Tarsus.Length", "Wing.Length", "Tail.Length"),
    ~ mean(.x, na.rm = TRUE)
  ))
...
#######################################################################
#GLMM for plot
beak_culmen_glmm <- lmer(
  values ~ ind + (1 | BirdLife.order / BirdLife.family),
  data = beak_culmen2
)
...
label_beak_culmen_stat <- substitute(
  paste("GLMM: ", italic("F"), " = ", fvalue, ", ", italic("p"), " = ", stat),
  list(
    stat = pvalue(beak_culmen_glmm_output$`Pr(>F)`),
...
label_tail_length_stat <- substitute(
  paste("GLMM: ", italic("F"), " = ", fvalue, ", ", italic("p"), " = ", stat),
  list(
    stat = pvalue(tail_length_glmm_output$`Pr(>F)`),
...
tarsus_length_stat <- t.test(
  tarsus_length$Field,
  tarsus_length$Museum,
  paired = F,
  alternative = "two.sided"
)
...
label_tarsus_length_stat <- substitute(
  paste("GLMM: ", italic("F"), " = ", fvalue, ", ", italic("p"), " = ", stat),
  list(
    stat = pvalue(tarsus_length_glmm_output$`Pr(>F)`),
...
label_wing_length_stat <- substitute(
  paste("GLMM: ", italic("F"), " = ", fvalue, ", ", italic("p"), " = ", stat),
  list(
    stat = pvalue(wing_length_glmm_output$`Pr(>F)`),
...
ggsave(
  filename = paste0(figFolder, 'Figure S5.pdf'),
  width = 14,
  height = 14,
  dpi = 400,
  units = "in",
...
ggsave(
  filename = paste0(figFolder, 'Figure S5.jpg'),
  width = 14,
  height = 14,
  dpi = 600,
  units = "in",
