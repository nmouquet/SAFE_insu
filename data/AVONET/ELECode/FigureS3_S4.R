...
beak_culmen_cor_s <- round(
  cor(data_beak_culmen_s$Beak.culmen.x, data_beak_culmen_s$Beak.culmen.y),
  3
)
...
beak_culmen_cor_ns <- round(
  cor(data_beak_culmen_ns$Beak.culmen.x, data_beak_culmen_ns$Beak.culmen.y),
  3
)
...
beak_nares_cor_s <- round(
  cor(data_beak_nares_s$Beak.nares.x, data_beak_nares_s$Beak.nares.y),
  3
)
...
beak_nares_cor_ns <- round(
  cor(data_beak_nares_ns$Beak.nares.x, data_beak_nares_ns$Beak.nares.y),
  3
)
...
beak_width_cor_s <- round(
  cor(data_beak_width_s$Beak.width.x, data_beak_width_s$Beak.width.y),
  3
)
...
beak_width_cor_ns <- round(
  cor(data_beak_width_ns$Beak.width.x, data_beak_width_ns$Beak.width.y),
  3
)
...
beak_depth_cor_s <- round(
  cor(data_beak_depth_s$Beak.depth.x, data_beak_depth_s$Beak.depth.y),
  3
)
...
beak_depth_cor_ns <- round(
  cor(data_beak_depth_ns$Beak.depth.x, data_beak_depth_ns$Beak.depth.y),
  3
)
...
#Calculate mean_Var for each trait, correlation coefficient and number of samples
#Standardised methods
data_tarsus_length_s <- subset(
  tarsus_length_split,
  Protocol.both.pairs.x == '1'
)
...
tarsus_length_cor_s <- round(
  cor(
    data_tarsus_length_s$tarsus.length.x,
    data_tarsus_length_s$tarsus.length.y
  ),
  3
)

#Non-standardised methods
data_tarsus_length_ns <- subset(
  tarsus_length_split,
  Protocol.both.pairs.x == '0'
)
...
tarsus_length_cor_ns <- round(
  cor(
    data_tarsus_length_ns$tarsus.length.x,
    data_tarsus_length_ns$tarsus.length.y
  ),
  3
)
...
scatter_tarsus_length_s <-
  ...
  annotate(
    ...
    label = paste(
      ...
      tarsus_length_cor_s,
      italic_n,
      tarsus_length_sample_numbers_s,
      sep = '~'
    ),
    x = -Inf,
    y = -Inf,
...
scatter_tarsus_length_ns <-
  ...
  annotate(
    ...
    label = paste(
      italic_R,
      tarsus_length_cor_ns,
      italic_n,
      tarsus_length_sample_numbers_ns,
      sep = '~'
    ),
    x = -Inf,
    y = -Inf,
...
wing_length_cor_s <- round(
  cor(data_wing_length_s$wing.length.x, data_wing_length_s$wing.length.y),
  3
)
...
wing_length_cor_ns <- round(
  cor(data_wing_length_ns$wing.length.x, data_wing_length_ns$wing.length.y),
  3
)
...
tail_length_cor_s <- round(
  cor(data_tail_length_s$tail.length.x, data_tail_length_s$tail.length.y),
  3
)
...
tail_length_cor_ns <- round(
  cor(data_tail_length_ns$tail.length.x, data_tail_length_ns$tail.length.y),
  3
)
...
plot_grid(
  ...
  ncol = 6,
  nrow = 2,
  rel_widths = c(0.05, 1, 1, 1, 1, 0.05, 0.05, 1, 1, 1, 1, 0.05),
  scale = 0.95
) +
  draw_label(
    label_beak_culmen_s,
...
ggsave(
  filename = paste0(figFolder, 'Fig S3 - Standardised methods.pdf'),
  width = 18,
  height = 12,
  dpi = 600,
  units = "in",
...
ggsave(
  filename = paste0(figFolder, 'Fig S4 - Non-standardised methods.pdf'),
  width = 18,
  height = 12,
  dpi = 600,
  units = "in",
