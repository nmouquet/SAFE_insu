...
plotTable$ltySpecimen[which(
  plotTable$Reference == "Dunn et al. 2001 Evolution"
)] <- 2
...
plotTable$Year[which(
  plotTable$Reference == "Pigot et al 2020 Nature Ecol Evol"
)] <- 2020.5

#stretch out recent years to aid visualisation
plotTable$Year2 <- plotTable$Year
plotTable$Year2[which(plotTable$Year >= 2015)] <- 2015 +
  5 * (plotTable$Year2[which(plotTable$Year >= 2015)] - 2015)
...
plot(
  1,
  1,
  xlim = c(min(possYears), max(possYears)),
  ylim = c(-10000, 13000),
  log = "x",
  yaxt = "n",
  ylab = "",
...
for (i in 1:length(yvals)) {
  lines(
    c(min(possYears) - 5, max(possYears)),
    c(yvals[i], yvals[i]),
    lwd = 0.25,
    col = "grey90"
  )
...
for (x in 1:nrow(plotTable)) {
  if (plotTable$openSource[x] == 0) {
    lines(
      c(plotTable$Year2[x], plotTable$Year2[x]),
      (c(0, plotTable$NumSpecies[x])),
      col = "grey50",
      lty = plotTable$ltySpecies[x]
    )
    lines(
      c(plotTable$Year2[x], plotTable$Year2[x]),
      -(c(0, plotTable$NumSpecimen[x])) / 10,
      col = "grey50",
      lty = plotTable$ltySpecimen[x]
    )
  }
...
for (x in 1:nrow(plotTable)) {
  if (plotTable$openSource[x] == 0) {
    points(
      plotTable$Year2[x],
      (plotTable$NumSpecies[x]),
      pch = 21,
      bg = "white",
      col = "grey50",
      cex = plotTable$SymbolSize[x]
    )
    points(
      plotTable$Year2[x],
      -(plotTable$NumSpecimen[x]) / 10,
      pch = 21,
      bg = "grey50",
      col = "grey50",
      cex = 0.5
...
yVals <- c(
  rev(seq(3500, 10500, length.out = 10)),
  seq(-2500, -9500, length.out = 10)
)
for (i in 1:length(studyNames)) {
  text(1957, yVals[i], studyNames[i], pos = 4, col = cols[i], cex = 0.5)
}
...
axis(
  1,
  at = seq(1960, 2045, 5),
  labels = rep("", length(seq(1960, 2045, 5))),
  cex.axis = 0.5,
  padj = -4,
  tck = -0.01
)
