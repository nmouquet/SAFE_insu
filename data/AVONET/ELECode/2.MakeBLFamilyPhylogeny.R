...
familyPhyTips$tip.label <- familyData$Family[match(
  familyPhy$tip.label,
  familyData$Species
)]
...
focEdge <- which(
  familyPhyTips$edge[, 2] == which(familyPhyTips$tip.label == "Bombycillidae")
)
focNode <- familyPhyTips$edge[focEdge, 1]
focEdgeLength <- familyPhyTips$edge.length[focEdge]
familyPhyTipsUpdate <- bind.tip(
  tree = familyPhyTips,
  tip.label = "Elachuridae",
  edge.length = focEdgeLength,
  where = focNode
)
...
write.nexus(
  familyPhyTipsL,
  file = paste0(PhylogeneticDataFolder, 'Birdlife_Family_Phylogeny.nex')
)
