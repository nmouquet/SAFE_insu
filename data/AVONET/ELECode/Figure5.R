...
datBL <- datBL[, match(
  c("Beak.Length_Culmen", "Order1", "Family1", "Species1"),
  names(datBL)
)]
dat <- dat[, match(
  c("Beak.Length_Culmen", "Order1", "Family1", "Species1_BirdLife"),
  names(dat)
)]
...
m <- lme(
  log10(Beak.Length_Culmen) ~ 1,
  random = ~ 1 | Order1 / Family1 / Species1_BirdLife,
  data = dat,
  method = "REML",
  control = (msMaxIter = 10000000)
)
...
for (i in 1:4) {
  ...
  if (i == 1) {
    axis(
      1,
      at = seq(-0.15, 0.15, 0.05),
      labels = round(seq(-0.15, 0.15, 0.05), 2),
      tck = -0.01,
      padj = -1.5
    )
  ...
  if (i == 2 | i == 4) {
    axis(
      1,
      at = seq(-0.15, 0.15, 0.05),
      labels = rep("", length(seq(-0.15, 0.15, 0.05))),
      tck = -0.01,
      padj = -1.5
    )
  }
...
axis(
  2,
  at = seq(0, 0.06, 0.015),
  labels = c("0.000", "0.015", "0.030", "0.045", "0.060"),
  tck = -0.025,
  las = 2,
  hadj = 0.75
)
...
mtext(
  "Among species (n = 10817)\nwithin families (n = 156)",
  side = 3,
  line = -1.5,
  cex = 0.75,
  at = 45
...
axis(
  2,
  at = seq(0, 0.025, 0.005),
  labels = c("0.000", "0.005", "0.010", "0.015", "0.020", "0.025"),
  tck = -0.025,
  las = 2,
  hadj = 0.75
)
...
mtext(
  "Among specimens (n = 61954)\nwithin species (n = 7072)",
  side = 3,
  line = -1.5,
  cex = 0.75,
  at = 2200
