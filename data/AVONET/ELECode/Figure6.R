...
dat$Primary.Lifestyle <- factor(
  dat$Primary.Lifestyle,
  levels = c("Aerial", "Aquatic", "Insessorial", "Terrestrial", "Generalist")
)
dat$Trophic.Niche <- factor(
  dat$Trophic.Niche,
  levels = c(
    "Nectarivore",
    "Herbivore aquatic",
...
dat$bill.res <- lm(
  log(dat$Beak.Length_Culmen) ~ log(dat$Mass),
  data = dat
)$resid
...
gridB@data$HWI_median_perCell[match(
  names(HWI_median_perCell),
  gridB@data$WorldID
)] <- as.numeric(HWI_median_perCell)
gridB@data$T_median_perCell <- NA
gridB@data$T_median_perCell[match(
  names(T_median_perCell),
  gridB@data$WorldID
)] <- as.numeric(T_median_perCell)
gridB@data$BL_median_perCell <- NA
gridB@data$BL_median_perCell[match(
  names(BL_median_perCell),
  gridB@data$WorldID
)] <- as.numeric(BL_median_perCell)

# set scale and colors
brks <- quantile(
  gridB@data$HWI_median_perCell,
  probs = seq(0, 1, 0.02),
  na.rm = T
)
gridB@data$col_HWI_median <- NA
gridB@data$col_HWI_median <- findInterval(
  gridB@data$HWI_median_perCell,
  brks,
  all.inside = TRUE
)
...
brks <- quantile(
  gridB@data$T_median_perCell,
  probs = seq(0, 1, 0.02),
  na.rm = T
)
gridB@data$col_T_median <- NA
gridB@data$col_T_median <- findInterval(
  gridB@data$T_median_perCell,
  brks,
  all.inside = TRUE
)
...
brks <- quantile(
  gridB@data$BL_median_perCell,
  probs = seq(0, 1, 0.02),
  na.rm = T
)
gridB@data$col_BL_median <- NA
gridB@data$col_BL_median <- findInterval(
  gridB@data$BL_median_perCell,
  brks,
  all.inside = TRUE
)
...
Legend <- function(breaks = NULL, col = NULL, axis.args = NULL) {
  ...
  axis.args <- c(
    list(side = 1, padj = -1, mgp = c(3, 1, 0), las = 0),
    axis.args
  )
...
HWIMapBreaks <- quantile(
  gridB@data$HWI_median_perCell,
  probs = seq(0, 1, 0.01),
  na.rm = T
)
HWIBoxplotMedian <- sapply(
  split(dat$Hand.Wing.Index, dat$Primary.Lifestyle),
  median,
  na.rm = T
)
...
TBoxplotMedian <- sapply(
  split(dat$tarsus.res, dat$Primary.Lifestyle),
  median,
  na.rm = T
)
...
BLMapBreaks <- quantile(
  gridB@data$BL_median_perCell,
  probs = seq(0, 1, 0.01),
  na.rm = T
)
BLBoxplotMedian <- sapply(
  split(dat$bill.res, dat$Trophic.Niche),
  median,
  na.rm = T
)
...
p1 <- ggplot(
  data = dat,
  aes(x = Primary.Lifestyle, y = Hand.Wing.Index, fill = Primary.Lifestyle)
) +
  geom_boxplot() +
  theme_classic() +
...
p2 <- ggplot(
  data = dat,
  aes(x = Primary.Lifestyle, y = tarsus.res, fill = Primary.Lifestyle)
) +
  geom_boxplot() +
  theme_classic() +
  scale_fill_manual(values = cols.tars)

# Beak plot:
p3 <- ggplot(
  data = dat,
  aes(x = Trophic.Niche, y = bill.res, fill = Trophic.Niche)
) +
  geom_boxplot() +
  theme_classic() +
  scale_fill_manual(values = cols.bill)
...
p2 <- p2 +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, color = "black")) +
  labs(x = NULL, y = "Relative tarsus length") +
  theme(legend.position = 'none')

p3 <- p3 +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, color = "black")) +
  labs(x = NULL, y = "Relative beak length") +
  theme(legend.position = 'none')
