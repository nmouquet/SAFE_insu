...
plotTree.wMultipleBars <- function(
  tree,
  xT1,
  xT2,
  xT3,
  ...
  for (i in 1:length(xT1)) {
    ...
    x1 <- dx +
      (wT1 / 2) * cos(pi / 2 - theta) -
      s * min(0, min(xT1)) * cos(theta) #top left
    x2 <- dx -
      (wT1 / 2) * cos(pi / 2 - theta) -
      s * min(0, min(xT1)) * cos(theta) #bottom left

    y1 <- dy -
      (wT1 / 2) * sin(pi / 2 - theta) -
      s * min(0, min(xT1)) * sin(theta) #top left
    y2 <- dy +
      (wT1 / 2) * sin(pi / 2 - theta) -
      s * min(0, min(xT1)) * sin(theta) #bottom left
    ...
    if (xT1[i] >= ThresholdT1) {
      polygon(
        c(x1, x2, x3Max, x4Max),
        c(y1, y2, y3Max, y4Max),
        col = makeTransparent("grey80", alpha = 100),
        border = FALSE
      )
      polygon(
        c(x1, x2, x3, x4),
      ...
      polygon(
        c(x1, x2, x3, x4),
        c(y1, y2, y3, y4),
        col = makeTransparent(colT1[i], alpha = alphaVal),
        border = FALSE
      )
    }
  }
  ...
  for (i in 1:length(xT1)) {
    ...
    x1 <- dx +
      (wT2 / 2) * cos(pi / 2 - theta) -
      s * min(0, min(xT2)) * cos(theta) #top left
    x2 <- dx -
      (wT2 / 2) * cos(pi / 2 - theta) -
      s * min(0, min(xT2)) * cos(theta) #bottom left

    y1 <- dy -
      (wT2 / 2) * sin(pi / 2 - theta) -
      s * min(0, min(xT2)) * sin(theta) #top left
    y2 <- dy +
      (wT2 / 2) * sin(pi / 2 - theta) -
      s * min(0, min(xT2)) * sin(theta) #bottom left
    ...
    if (xT2[i] >= ThresholdT2) {
      ...
      polygon(
        c(x1, x2, x3, x4),
        c(y1, y2, y3, y4),
        col = makeTransparent(colT2[i], alpha = alphaVal),
        border = FALSE
      )
    }
  }
  ...
  for (i in 1:length(xT1)) {
    ...
    x1 <- dx +
      (wT3 / 2) * cos(pi / 2 - theta) -
      s * min(0, min(xT3)) * cos(theta) #top left
    x2 <- dx -
      (wT3 / 2) * cos(pi / 2 - theta) -
      s * min(0, min(xT3)) * cos(theta) #bottom left

    y1 <- dy -
      (wT3 / 2) * sin(pi / 2 - theta) -
      s * min(0, min(xT3)) * sin(theta) #top left
    y2 <- dy +
      (wT3 / 2) * sin(pi / 2 - theta) -
      s * min(0, min(xT3)) * sin(theta) #bottom left
    ...
    if (xT3[i] >= ThresholdT3) {
      ...
      polygon(
        c(x1, x2, x3, x4),
        c(y1, y2, y3, y4),
        col = makeTransparent(colT3[i], alpha = alphaVal),
        border = FALSE
      )
    }
  }
