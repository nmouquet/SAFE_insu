...
circleSize <- findInterval(
  museumData$Specimens.sampled,
  circleSizeBrks,
  all.inside = TRUE
)
...
countries@data$Nspecimens <- SpecPerCountry[match(
  countries@data$name,
  names(SpecPerCountry)
)]
...
class_of_country <- cut(
  countries@data$Nspecimens,
  breaks = c(0, 1, 10, 50, 100, 1000, 10000)
)
...
gridB@data$TotalRichness[match(
  RichnessTable$WorldID,
  gridB@data$WorldID
)] <- RichnessTable$TotalRichness
# sampled species richness
gridB@data$SampledRichness <- NA
gridB@data$SampledRichness[match(
  RichnessTable$WorldID,
  gridB@data$WorldID
)] <- RichnessTable$SampledRichness
# proportion of species sampled
gridB@data$PropRichnessSampled <- gridB@data$SampledRichness /
  gridB@data$TotalRichness

# set breaks and colors
brks <- quantile(
  gridB@data$PropRichnessSampled,
  probs = seq(0, 1, 0.01),
  na.rm = T
)
gridB@data$SamplingCol <- NA
gridB@data$SamplingCol <- findInterval(
  gridB@data$PropRichnessSampled,
  brks,
  all.inside = TRUE
)
